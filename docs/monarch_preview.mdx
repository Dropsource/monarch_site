...
However, implementing the Preview isolation was hard. It meant a lot of hard work for us.
- **Monarch has to manage native windows** on every platform it runs, which means maintaining native code on every platform
- Since the Preview and Controller windows are separate process, their **communication** **is non-trivial**.
- Which means that the Monarch **build process is also non-trivial.**

Again, Monarch was hard work for us, and we are so happy we chose this path: your previews are very well isolated which means you can do more and, we can do more for you.
