---
authors: schikwiri
slug: play-with-page-transitions
...
To demonstrate how isolation makes your work as a developer easier, let's write some [stories](/docs/write-first-story) for the `ExpenseListWidget` shown
...
First, let's set up the `ExpenseListWidget`. It exposes a `transitionBuilder` callback whose return value we can change around to
get various animations.
...
Next, let's try a transition that slides in the next page from the bottom. To do that, we'll change the `TransitionBuilder`
callback to return a `SlideTransition` animated widget that takes the child parameter and an offset animation with begin and end
offsets set to `Offset(0.0, 1.0)` and `Offset.zero`, respectively, like below:
...
Finally, to try out a right-to-left slide transition, we’ll have a callback similar to the previous one, except in this, the begin
offset value in the `Tween` is set to `Offset(1.0,0.0)` like below:
...
<div className="blog-video video">
  <div className="text--center">
    <video
      ...
      controls
      autoPlay
      src="/assets/monarch_interactions.webm"
      type="video/webm"
    />
  </div>
</div>
...
The complete code is available [here](https://github.com/Dropsource/monarch_samples).
